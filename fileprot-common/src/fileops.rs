...
///
/// The syscall is offloaded to tokio's blocking thread pool for consistency
/// with the rest of the fileops API.
pub async fn open_o_path(path: &Path) -> ah::Result<OwnedFd> {
...
pub async fn open_dir_components(path: &Path) -> ah::Result<OwnedFd> {
    let path = path.to_path_buf();
...
/// Return `(st_dev, st_ino)` for the directory or file referenced by `fd`.
///
/// The syscall is offloaded to tokio's blocking thread pool for consistency
/// with the rest of the fileops API.
...
///
/// The walk loop — one `openat("..")` + `fstatat` per level up to the VFS
/// root — is offloaded to tokio's blocking thread pool to avoid stalling the
/// async runtime on deep trees or slow filesystems.
...
fn is_fd_inside_blocking(child_fd: OwnedFd, ancestor_id: (u64, u64)) -> ah::Result<bool> {
    ...
    loop {
        let cur_id = fd_id_blocking(current.as_fd())?;
